  ...
  return <>
    ...
    {development?.graphRunId && <section className="development-run" aria-label="开发图运行">
      ...
      <div className="development-reduce-flow">
        ...
        {development.interruptKind === "branch_review" && development.interruptId && <article><strong>分支审核等待人工决定</strong><p>只会批准当前中断明确列出的隔离分支。</p><button type="button" disabled={releasePending || development.pendingBranchIds.length === 0} onClick={() => resumeDevelopment({ decisions: Object.fromEntries(development.pendingBranchIds.map((branchId) => [branchId, "approved"])) })}>{releasePending ? "审批已提交" : "批准分支审核并继续"}</button></article>}
