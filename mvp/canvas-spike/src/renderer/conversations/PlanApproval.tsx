...
export function PlanApproval({ sessionId, onApproved }: { sessionId: string; onApproved(plan: ResearchPlan): void }) {
  ...
  const planStatus = plan.status === "queued" ? "已批准 · 排队执行" : plan.status === "approved" ? "已批准" : "等待批准";
  return <section className="research-plan" aria-label="执行计划">
    <header><div><small>Research Blueprint · v{plan.version}</small><h3>{plan.goal}</h3></div><span>{planStatus}</span></header>
