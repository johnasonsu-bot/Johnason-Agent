...
export function ConversationWorkspace() {
  ...
  const send = (prompt: string) => {
    ...
    void conversationApi.sendMessage(sessionId, prompt, `ui-${sessionId}-${++sequence}`).then((result) => { applyEvents(result.events ?? []); setSource("Task 3 REST/SSE"); setStatus("已完成 · Completed"); }).catch(() => {
  ...
  return <section aria-label="会话工作区" style={{ display: "flex", minHeight: "calc(100vh - 105px)", borderTop: "1px solid #e6e5e2", background: "#fff" }}>
    ...
    <div style={{ flex: 1, minWidth: 0, display: "grid", gridTemplateRows: "auto auto 1fr auto" }}><div data-testid="conversation-source" style={{ padding: "6px 18px", color: "#6b7280", fontSize: 12 }}>来源 · {source} · session {sessionId}</div><div data-testid="conversation-status" aria-live="polite" style={{ padding: "4px 18px", color: status.startsWith("执行中") ? "#00a9f4" : "#6b7280", fontSize: 12 }}>{status}</div><Timeline entries={entries} group={group.map((role) => `${role} · ${role === "产品经理" ? "Product Manager" : role === "架构师" ? "Architect" : role === "工程师" ? "Engineer" : role}`)} /><Composer onSend={send} pending={pending} /></div>
